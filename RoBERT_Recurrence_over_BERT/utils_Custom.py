...
def evaluate(target, predicted):
    ...
    ac = accuracy(predicted, target, threshold=THRESHOLD)   
...
#             lengt=lengt_batch[doc]
        ...
        fin_outputs.append(torch.softmax(
            outputs, dim=1))
    return torch.concat(fin_outputs), torch.concat(fin_targets), losses
...
#             lengt=lengt_batch[doc]
        ...
        fin_targets.append(targets)
        fin_outputs.append(soft_out)
        
    fin_outputs = torch.concat(fin_outputs)
    fin_targets = torch.concat(fin_targets)
    #fin_targets = torch.reshape(fin_targets, fin_outputs.shape)
