...
class ConsumerComplaintsDataset1(Dataset):
    ...
    def __init__(self, tokenizer, max_len,  get_popular_keys = True, chunk_len=200, overlap_len=50, approach="all", max_size_dataset=None, file_location="./dataset.csv", min_len=249, max_chunk_num=5):
        self.tokenizer = tokenizer
        self.get_popular_keys = get_popular_keys
    ...
    def process_data(self, file_location):
        ...
        if self.get_popular_keys:
            ...
            keys = [key[0] for key in sorted_stats[:5]]
            print(keys)

            df_new = pd.DataFrame(columns=('path', 'labels'))

            for index, row in df.iterrows():
                new_label = []
                for key in row['labels'].split(","):
                    if key in keys:
                        new_label.append(key)
                if len(new_label) > 0:
                    df_new.loc[index] = [row['path'], ','.join(new_label)]
                    ','.join(l)
        ...
        if(self.max_size_dataset):
            df = df.loc[0:self.max_size_dataset, :]
        
        # training del mlb per passare da ['classe_stringa'] a [0 0 0 0 1 0 0 0]
        labels = []
        for label in df["labels"]:
            #print(label.split(","))
        ...
        self.mlb = self.mlb.fit(labels)
        
        
        ## viene usata per sistemare il dataset di partenza, a noi non serve
...
#         train['text'] = train.text.apply(self.clean_txt)
        ...
        return df['path'].values, df['labels'].values
    ...
    def long_terms_tokenizer(self, data_tokenize, targets):
        ...
        if len(input_ids_list) > self.max_chunk_num:
            input_ids_list = input_ids_list[:self.max_chunk_num]
            attention_mask_list = attention_mask_list[:self.max_chunk_num]
            token_type_ids_list = token_type_ids_list[:self.max_chunk_num]
            targets_list = targets_list[:self.max_chunk_num]
    ...
    def __getitem__(self, idx):
        ...
        with open(f"{self.data[idx]}_content.txt", "r") as f:
            content = f.read()
            content = self.clean_txt(content)
        
        targets = sum(self.mlb.transform([self.label[idx].split(",")]))
